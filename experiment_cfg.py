...
import json
import pickle
from model_build import SDFNet
from tqdm import tqdm
from API import *
from utils import *
import torch.distributed

class Experiment_cfg:
    def __init__(self, args):
        super(Experiment_cfg, self).__init__()
        self.args = args
    ...
    def _model(self):
        args = self.args
        self.model = SDFNet( shape_in=tuple(args.in_shape),
                            shape_out=tuple(args.out_shape),
                            hid_channel=args.hid_channel,
                            L_layer=args.L_layer,
                            encoder_kernel_size=tuple(args.encoder_kernel_size),
                            layer_config=tuple(args.layer_config),
                            reduction=args.reduction).to(self.device)

        if os.path.exists(self.path):
            self.model.load_state_dict(torch.load(self.path + '/' + 'MNist_model.pth'),strict=False)
            print('success to load pretrain-model！')
